...
/**
 * 
 *  Obtiene todos pedidos entregados
 * 
 */
const generateVentasReport = async ( orders = [], from, to ) => {
    let total = 0;
    const cantPedidos = orders.length;
    const pedidos = orders.map( (v) => {
        total+=v.importe;
        let f = new Date(v.fecha).toLocaleDateString('en-US');
        return `
        <tr>
            <td class="no">${v.id}</td>
            <td class="desc">${v.cliente}</td>
            <td class="qty">${f}</td>
            <td class="unit">${v.hora}</td>
            <td class="total">${v.importe}</td>
        </tr>
        `;
    });
    const content = `
        ...
        <header class="clearfix">
            <div class="container">
                <figure>
                    <img class="logo" src="https://res.cloudinary.com/dlds4xwpk/image/upload/v1606442600/pedidos_mobile_data/logo_cvf1ok.png" alt="">
                </figure>
            </div>
        </header>
        ...
        <section>
            <div class="details clearfix">
                <div class="client left">
                    <p>INFORMACION DEL REPORTE</p>
                    <p>
                        Reporte de ventas de (X) fecha a (Y) fecha. 
                    </p>
                    <p>Total de pedidos entregados: ${cantPedidos}</p>
                </div>
                    ...
                    <table>
                        <tbody class="head">
                            <tr>
                                ...
                                <th class="qty"><div>Fecha</div></th>
                                <th class="unit"><div>hora</div></th>
                ...
                <div class="no-break">
                    <table class="grand-total">
                        <tbody>
                            ...
                            <tr>
                                <td class="grand-total" colspan="5"><div><span>GRAN TOTAL:</span>\$${total}</div></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <footer>
            <div class="container">
                <div class="thanks">Fin del reporte.</div>
                <div class="end"></div>
            </div>
        </footer>
    `;
    ...
    return new Promise( async(resolve, reject) => {
       await pdf.create(content).toFile(`${__dirname}/archives/others/clientbyid.pdf`, (err, res) => {
